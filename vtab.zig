...
const StringSliceFromCPointerError = error{} || mem.Allocator.Error;

/// Converts an array of arguments coming from sqlite to a slice of slice which makes it easier to use.
fn stringSliceFromCPointer(allocator: mem.Allocator, argc: c_int, argv: [*c]const [*c]const u8) StringSliceFromCPointerError![]const []const u8 {
    const size = @intCast(usize, argc);

    var res = try allocator.alloc([]const u8, size);
    for (res) |*slice, i| {
        // The documentation of sqlite says each string in argv is null-terminated
        slice.* = mem.sliceTo(argv[i], 0);
    }
...
fn validateTableType(comptime Table: type) void {
    // Validate the `init` function
    {
        ...
        const error_message =
            \\the Table.init function must have the signature `fn init(allocator: std.mem.Allocator, diags: *sqlite.vtab.VTabDiagnostics) InitError!*Table`
        ;
        ...
        if (info.args[2].arg_type.? != []const []const u8) @compileError(error_message);
...
pub fn VirtualTable(
    ...
    return struct {
        ...
        fn createState(allocator: mem.Allocator, diags: *VTabDiagnostics, module_context: *ModuleContext, args: []const []const u8) !*State {
        ...
        fn xConnect(db: ?*c.sqlite3, module_context_ptr: ?*anyopaque, argc: c_int, argv: [*c]const [*c]const u8, vtab: [*c][*c]c.sqlite3_vtab, err_str: [*c][*c]const u8) callconv(.C) c_int {
            ...
            // TODO(vincent): maybe we should provide a way to automatically parse arguments
            const args = stringSliceFromCPointer(arena.allocator(), argc, argv) catch {
...
const TestVirtualTable = struct {
    ...
    pub fn init(gpa: mem.Allocator, diags: *VTabDiagnostics, args: []const []const u8) InitError!*TestVirtualTable {
        ...
        const rows = blk: {
            ...
            for (args) |arg| {
                if (mem.startsWith(u8, arg, "content=")) {
                    const pos = mem.indexOfScalar(u8, arg, '=') orelse continue;
                    content = arg[pos + 1 ..];
                } else if (mem.startsWith(u8, arg, "n=")) {
                    const pos = mem.indexOfScalar(u8, arg, '=') orelse continue;
                    const value = arg[pos + 1 ..];
                    n = fmt.parseInt(usize, value, 10) catch |err| {
                        switch (err) {
                            error.InvalidCharacter => diags.setErrorMessage("not a number: {s}", .{value}),
                            else => diags.setErrorMessage("got error while parsing value {s}: {!}", .{ value, err }),
                        }
                        return err;
                    };
                }
            ...
            // TODO-vincent): stop using random data so we can actually test
...
test "virtual table" {
    ...
    try db.exec("CREATE VIRTUAL TABLE foobar USING myvtab(content=hello, n=200)", .{ .diags = &diags }, .{});
    ...
    // const row1 = rows[0];
    // try testing.expectEqual(@as(i64, 0), row1.id);
    // try testing.expectEqualStrings("foo_hello", row1.foo);
    // try testing.expectEqualStrings("bar_hello", row1.bar);
    // try testing.expectEqual(@as(usize, 0), row1.baz);

    // const row2 = rows[1];
    // try testing.expectEqual(@as(i64, 1), row2.id);
    // try testing.expectEqualStrings("foo_hello", row2.foo);
    // try testing.expectEqualStrings("bar_hello", row2.bar);
    // try testing.expectEqual(@as(usize, 1), row2.baz);
}
